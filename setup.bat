...
node -e "const fs=require('fs');const path=require('path');const os=require('os');const localAppData=process.env.LOCALAPPDATA||'';const appData=process.env.APPDATA||'';let configDir='';const pkgDir=path.join(localAppData,'Packages');if(fs.existsSync(pkgDir)){const dirs=fs.readdirSync(pkgDir).filter(d=>d.startsWith('Claude_'));if(dirs.length>0)configDir=path.join(pkgDir,dirs[0],'LocalCache','Roaming','Claude')}if(!configDir&&fs.existsSync(path.join(appData,'Claude'))){configDir=path.join(appData,'Claude')}if(!configDir){console.log('  Claude Desktop が見つかりません。MCPサーバーの設定はスキップします。');process.exit(0)}const configPath=path.join(configDir,'claude_desktop_config.json');let c={};try{c=JSON.parse(fs.readFileSync(configPath,'utf-8'))}catch{}if(c.mcpServers&&c.mcpServers['dns-register']){console.log('  MCPサーバーは既に設定済みです。');process.exit(0)}if(!c.mcpServers)c.mcpServers={};const batPath=path.join(process.cwd(),'start-mcp.bat');c.mcpServers['dns-register']={command:'cmd',args:['/c',batPath],cwd:process.cwd()};fs.writeFileSync(configPath,JSON.stringify(c,null,2),'utf-8');console.log('  MCPサーバーの設定を追加しました。')"
