...
1. IT部門から受け取った `setup.bat` をデスクトップなどに保存してください
2. `setup.bat` をダブルクリックしてください
   - Git と Node.js の確認が行われます（Node.js が未インストールの場合は自動でインストールされます）
   - プロジェクトが自動でダウンロードされます
3. ダウンロードされた `route53_dns_auto_register` フォルダ内の `.env` ファイルに AWS 認証情報を設定してください（下記参照）
4. Claude Desktop の Cowork で `route53_dns_auto_register` フォルダをワークスペースとして開いてください
5. 「登録して」と伝えるだけで操作が開始されます

> セットアップは初回のみ必要です。以降は Cowork を起動するだけで使えます。Cowork が起動時に自動で最新化（`git pull` + ビルド）を行います。
