...
初回のみ、スキルファイルのアップロードが必要です。

1. Claude Desktop の画面左下にある「カスタマイズ」をクリックします
2. 「スキル」タブを選択します
3. 「+」ボタンをクリックし、「スキルをアップロード」を選択します
4. プロジェクトフォルダ内の `.claude/skills/dns-register/SKILL.md` をアップロードします

一度アップロードすれば、以降のセッションで自動的に有効になります。再アップロードは不要です。

> スキルファイルが更新された場合（`git pull` で変更があった場合）は、再度アップロードしてください。
...
スキルファイルは `.claude/skills/dns-register/SKILL.md` に配置されています。初回のみ Cowork の「カスタマイズ」>「スキル」からアップロードが必要です。一度アップロードすれば、以降のセッションで自動的に有効になります。
