...
│   └── skills/
│       └── dns-register/
│           └── SKILL.md     # CLIコマンド実行手順スキル
├── src/
│   ├── cli.ts           # CLI エントリポイント（Cowork + IT部門 共通）
...
├── docs/
│   └── architecture.md  # ソフトウェア構造図（本ファイル）
...
sequenceDiagram
    ...
    U->>C: はい
    C->>CLI: npx dns-register encode-name --shop-name "山岡家 札幌店" --shop-code s1105 --env-file .env
    ...
    U->>C: はい
    C->>CLI: npx dns-register create-records --shop-code s1105 --start-ip 192.168.94.65 --env-file .env
    CLI->>H: handleCreateRecords(params, route53Client, config)
    ...
    U->>C: はい
    C->>CLI: npx dns-register add-device --shop-code s1105 --device rt --ip 192.168.94.66 --env-file .env
    CLI->>H: handleAddDevice(params, route53Client, config)
    H->>R53: CNAME 登録
    R53-->>H: 成功
    H-->>CLI: AddDeviceResult
    CLI-->>C: コンソール出力（結果表示）
...
sequenceDiagram
    ...
    IT->>CLI: npx dns-register encode-name --shop-name "山岡家 札幌店" --shop-code s1105 --env-file .env
    CLI->>H: handleEncodeName(params, route53Client, config)
    H->>R53: TXT レコード登録
    R53-->>H: 成功
    H-->>CLI: EncodeNameResult
